...
int output_file(int fd, struct dbheader_t *dbhdr, struct  employee_t *employees){
    ...
    if(ftruncate(fd, sizeof(struct dbheader_t) + sizeof(struct employee_t) * nEmployees) != 0){
        printf("Error while shrinking the file. \n");
        return STATUS_ERROR;
    }
    return STATUS_SUCCESS;
}
...
int remove_employee(struct dbheader_t *dbhdr, struct employee_t **employeesOut, char* name){
    if(employeesOut == NULL){
        printf("Empty *employeesOut[] in remove_employee\n");
        return STATUS_ERROR;
    }
    if(dbhdr == NULL) return STATUS_ERROR;
    ...
    int newCount = dbhdr->count -1;
    struct employee_t *employees = calloc(newCount, sizeof(struct employee_t));
    int i = 0;
    int j = 0;

    for(i = 0; i < (newCount + 1); i++){
        if(strcmp(name, (*employeesOut)[i].name) != 0){
            employees[j++] = (*employeesOut)[i];
        }
    }
    if(i == j){
        printf("Name to be deleted not part of the database.\n");
            return STATUS_ERROR;
    }
    *employeesOut = realloc(*employeesOut, sizeof(struct employee_t) * newCount);
    *employeesOut = employees;
    dbhdr->count = newCount;
